...
def checkandstart_fioser(path, suitename):
    with open('{}/fioserver_list.conf'.format(path, suitename), 'r') as f:
        for client in f:
            ...
            while output == '':
                try:
                    output = subprocess.check_output('ssh {} ps -ef | grep fio | grep server | grep -v grep'.format(client), shell=True)
...
def create_log_dir(path, ceph_config):
    config = ''
...
def run(path, ceph_config={}):
    org_ceph_config = modify_ceph_config(ceph_config)
    print org_ceph_config
    log_dir = create_log_dir(path, ceph_config)
            ...
            if match_config:
                ...
                with open(log_file, 'wt') as handle:
                    while True:
                        ...
                        print line
                        handle.write(line)
                sysinfo.get_all_host_logfile('{}/sysinfo_{}'.format(log_dir, log_file_name))
                time.sleep(1)
            else:
                i = i + 1
    reset_ceph_config(org_ceph_config)
    return log_dir

def gen_result(log_dir, todb=False):
    print log_dir
    ...
    try:
        ...
        for sysinfo_dir in sysinfo_dir_list:
            sysinfo = SysInfo(havedb=todb)
            sysinfo.deal_with_sysinfo_logfile(
                sysinfo_dir,
                '{}/../../../ceph_hw_info.yml'.format(os.getcwd())
            )

def update_ceph_conffile(host, ceph_config):
...
def modify_ceph_config(ceph_config):
    sysinfo = SysInfo()
    org_all_config = {}
    for host in sysinfo.host_list:
        org_host_config = {}
        ...
        ssh.close()
    #update_ceph_conffile(host, ceph_config)
    return org_all_config

def reset_ceph_config(org_all_config):
...
def main():
    ...
    os.chdir(path)
    checkandstart_fioser(path, args.suitename)
    ...
    if os.path.exists(ceph_config_file): 
        ...
        for ceph_config in ceph_configs:
            log_dir = run(path, ceph_config=ceph_config)
            time.sleep(2)
            gen_result(log_dir, todb=args.todb)
    else:
        log_dir = run(path)
        time.sleep(2)
        gen_result(log_dir, todb=args.todb)
